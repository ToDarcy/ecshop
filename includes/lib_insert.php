...
function insert_ads($arr)
{
    ...
    foreach ($res AS $row)
    {
		...
		else if($row['position_name'] == '首页-倒计时广告')
		{
			$src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
							DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
			$ads[] = '<li> <a href="affiche.php?ad_id='.$row[ad_id].'&amp;uri='.urlencode($row['ad_link']). '" target="_blank"><img src="'.$src.'" width="' .$row['ad_width']. '" height="'.$row[ad_height].'"
					border="0" /></a>
      <p class="cle"><span class="temail_timer" showday="show" value="'.$row['end_time'].'"></p>
    </li>';
		}
		else
		{
...
function insert_right_collect()
{
	$user_id = $_SESSION['user_id'];
	$sql = 'SELECT g.goods_id, g.goods_name, g.market_price,goods_thumb, g.shop_price AS org_price, '.
				"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
			...
			" LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
				"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
			" WHERE c.user_id = '$user_id' ORDER BY c.rec_id DESC limit 5";
	...
	$output = $GLOBALS['smarty']->fetch('library/right_collect.lbi');
	return $output;
}
