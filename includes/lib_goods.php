...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number, g.market_price, g.shop_price, g.promote_start_date, g.promote_end_date, g.promote_price ' .
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.sales_volume_base, g.comments_number, g.goods_number, g.shop_price AS org_price, g.promote_price, ' .
...
$sort_order=$sort_order+1;
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.sales_volume_base, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    ...
    $cat['id']   = $cat_id;
	$sql = 'SELECT cat_desc FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = $cat_id";
    $cat['desc'] = $GLOBALS['db']->getOne($sql);
    $cat['url']  = build_uri('category', array('cid' => $cat_id), $cat['desc']);
    $cat['id']   = $cat_id;
	...
	foreach($cat_list_arr as $key=>$value)
	{
		if($value['level'] == 1)
		{
			 $sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.market_price, g.goods_number, g.shop_price AS org_price, ' .
					"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
					'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img '.
					'FROM '.$GLOBALS['ecs']->table('goods') .' AS g '.
					'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp '.
			...
			if ($num > 0)
			{
			 ...
			 foreach($goods_res as $idx=>$row)
			 {
					...
					$goods_res[$idx]['promote_price'] = $promote_price > 0 ? price_format($promote_price) : '';
					$goods_res[$idx]['shop_price']   = price_format($row['shop_price']);
					$goods_res[$idx]['goods_number']   = $row['goods_number'];
	...
	$sql = "SELECT b.brand_id, b.brand_name, brand_logo , COUNT(*) AS goods_num ".
			...
			"WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND b.is_show = 1 " .
			" AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
			"GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC LIMIT 6";
