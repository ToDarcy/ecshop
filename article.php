...
$sql="select article_id, cat_id, title, description, click_count, open_type, file_url from " . $GLOBALS['ecs']->table('article') . " $sql_where limit 0,5 ";
//echo $sql;
$res_art_rel=$GLOBALS['db']->query($sql);
while ( $row_art_rel = $GLOBALS['db']->fetchRow($res_art_rel) )
{
$row_art_rel['url']=$row_art_rel['open_type'] != 1 ? build_uri('article', array('aid'=>$row_art_rel['article_id']), $row_art_rel['title'], $row_art_rel['description'], $row_art_rel['click_count']) : trim($row_art_rel['file_url']);
