...
if (in_array($action, $ui_arr))
{
	...
	$smarty->assign('script_name',     filename());       // 当前文件名
}
...
elseif ($action == 'ajax_act_login')
{
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        if(strpos($back_act, 'user.php') != false || strpos($back_act, 'register.php') != false){
			$back_act = 'index.php';
		}
