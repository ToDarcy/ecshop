...
function send_mail($name, $email, $subject, $content, $type = 0, $notification = false)
{
    ...
    if (C('mail_charset') != CHARSET) {
        $name = ecs_iconv(CHARSET, C('mail_charset'), $name);
        $subject = ecs_iconv(CHARSET, C('mail_charset'), $subject);
        $content = ecs_iconv(CHARSET, C('mail_charset'), $content);
        $shop_name = ecs_iconv(CHARSET, C('mail_charset'), C('shop_name'));
    }
...
function log_write($arg, $file = '', $line = '')
{
    ...
    file_put_contents(DATA_PATH . '/log.txt', $str);
}
