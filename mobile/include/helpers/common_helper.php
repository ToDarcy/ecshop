...
function assign_query_info()
{
    ...
    $global->tpl->assign('gzip_enabled', $gzip_enabled);
}
...
{
    $global = getInstance();
    $sql = 'SELECT region_id, region_name FROM ' . $global->ecs->table('region') .
...
{
    $global = getInstance();
    $sql = 'SELECT brand_id, brand_name FROM ' . $global->ecs->table('brand') . ' ORDER BY sort_order';
    $res = $global->db->getAll($sql);
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $global->ecs->table('order_action') .
            ...
            'FROM ' . $global->ecs->table('order_info') . " WHERE order_sn = '$order_sn'";
    $global->db->query($sql);
}
...
function get_snatch_result($id)
{
    ...
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ' FROM ' . $global->ecs->table('snatch_log') . ' AS lg '.
            ' LEFT JOIN ' . $global->ecs->table('users') . ' AS u ON lg.user_id = u.user_id'.
    ...
    if ($rec)
    {
        ...
        $rec['order_count'] = $global->db->getOne($sql);
    }
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    $global->db->autoExecute($global->ecs->table('account_log'), $account_log, 'INSERT');
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ...
               " LEFT JOIN " . $global->ecs->table('article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    $sql = "SELECT `volume_number` , `volume_price`".
           " FROM " .$global->ecs->table('volume_price'). "".
...
function sort_goods_attr_id_array($goods_attr_id_array, $sort = 'asc')
{
    ...
    $sql = "SELECT a.attr_type, v.attr_value, v.goods_attr_id
            FROM " .$global->ecs->table('attribute'). " AS a
            LEFT JOIN " .$global->ecs->table('goods_attr'). " AS v
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$global->ecs->table('products'). " WHERE $_goods_id $conditions";
    $result_products = $global->db->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$global->ecs->table('goods_attr'). " WHERE $_goods_id";
    $result_goods_attr = $global->db->getAll($sql);
